# Mini GoLang Projects
These are Mini GoLang Projects that I have done and want to do!

* [ ] Number Guessing Game
* [ ] Credit Card Validator
* [ ] Unit Converter and Currency Converter
* [ ] GitHub User Activity
* [ ] Todo List API
* [x] Blog API
* [ ] Weather API
* [ ] Form Service
* [x] URL Shortener
* [ ] Basic Web Scraper
* [ ] Basic Web Crawler
* [ ] Concurrent Grep
* [ ] Replicated Memory Cache
* [ ] Caching Proxy
* [ ] Markdown Note-taking App
* [ ] Image Processing Service
* [ ] Pixel Art Editor
* [ ] Space Invaders Game: Exploring Graphics and Game Development

## References:
1. [Golang Practice Projects](https://zerotomastery.io/blog/golang-practice-projects/)
2. [Github User Activity](https://roadmap.sh/projects/github-user-activity)
3. [Number Guessing Game](https://roadmap.sh/projects/number-guessing-game)
5. [Weather API Wrapper Service](https://roadmap.sh/projects/weather-api-wrapper-service)
6. [TODO List API](https://roadmap.sh/projects/todo-list-api)
7. [Caching Server](https://roadmap.sh/projects/caching-server)
8. [Markdown Note Taking App](https://roadmap.sh/projects/markdown-note-taking-app)
9. [Image Processing Service](https://roadmap.sh/projects/image-processing-service)
10. [Space Invaders Game: Exploring Graphics and Game Development](https://www.geeksforgeeks.org/blogs/golang-projects-ideas-for-beginners/#10-space-invaders-game-exploring-graphics-and-game-development)
